...
| telegram_id | ім'я | location_ids | active | group_ids |
| --- | --- | --- | --- | --- |
| 5401968520 | Андрій Голубенко | pozniaky,kniazhyi-zaton | TRUE | |
| 123456789 | Юра | pozniaky | TRUE | poz-adults |
| 987654321 | Адміністратор | * | TRUE | |

Щоб дізнатися Telegram ID, тренер надсилає боту `/start`. Якщо доступу немає, бот
покаже його ID. `*` у `location_ids` дозволяє всі локації.

Опційна колонка `group_ids` обмежує групи (через кому). Якщо порожня — усі групи
вказаних локацій. Якщо в тренера **одна** доступна група, бот одразу відкриває вибір
дати (без кроків «локація → група»).
...
Бот показує для обліку **лише дати**, що збігаються з колонкою `день` (за останні
14 днів). Якщо на обрану дату в розкладі один час — він підставляється автоматично;
якщо кілька — бот запропонує вибір.
...
- `/start` відкриває меню: **Облік** / **Статистика** / **Додати учня**
...
Під час відмітки натисніть **➕ Додати учня** і надішліть ПІБ у відповідь на запит.
Бот додасть рядок в аркуш `Учні` і оновить список присутніх.

Якщо в тренера лише одна група:

```text
/add Іван Петренко
```

Якщо груп кілька — бот спочатку покаже вибір групи:
